...
public class Preview extends SurfaceView implements SurfaceHolder.Callback {
	...
	private void configure(Camera camera) {
		...
		Camera.Size maxSz = null;
		for (Size sz : sizes) {
			if (maxSz != null && (sz.width > maxSz.width && sz.height > maxSz.height)) {
				//do nothing
			} else {
				maxSz = sz;
			}
		...
		params.setPictureSize(maxSz.width, maxSz.height);
	...
	public void onPreviewFrame(byte[] data, Camera camera) {
		if (mMotionDetection.detect(data)) {
			...
			if (now > mReferenceTime + PICTURE_DELAY) {
				...
				*/
				count++;
				Log.i(TAG, "onPreviewFrame achieved " + count);
			} else {
